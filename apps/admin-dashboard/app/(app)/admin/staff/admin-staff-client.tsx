...
async function fetchAdminStaffBundle(): Promise<{
  ...
  return {
    allUsers: [...systemRows, ...staffByTenant.flat()],
    tenants: tenantsList,
  };
}
...
export default function AdminStaffPage() {
  ...
  const staffQuery = useQuery({
    queryKey: erpKeys.adminStaffUsers(),
    queryFn: fetchAdminStaffBundle,
  ...
  const refreshStaffUsers = () => {
    void queryClient.invalidateQueries({
      queryKey: ["erp", "admin", "staff-users"],
    });
  };
  ...
  const handleOpenEdit = (row: UnifiedUser) => {
    ...
    setActiveStaff({
      ...
      originalTenantSlug: row.tenantSlug,
      originalTenantName: row.tenantName,
    });
    setFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formMode === "create") {
        ...
        const sourceSlug = activeStaff.originalTenantSlug ?? slug;
        const tenantChanged =
          sourceSlug.toLowerCase() !== slug.toLowerCase();
        await updateStaff(sourceSlug, activeStaff.id, {
          name: activeStaff.name.trim() || undefined,
          email: activeStaff.email.trim() || undefined,
          password: activeStaff.password?.trim() || undefined,
          role: activeStaff.role.trim() || undefined,
          ...(tenantChanged ? { targetTenant: slug } : {}),
        });
      }
  ...
  const handleSubmitSystemUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (systemFormMode === "create") {
        ...
        await updateSystemUser(activeSystemUser.id, {
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="space-y-4 px-5 py-4">
                    <div className="space-y-1">
                      ...
                      {formMode === "edit" &&
                        activeStaff.originalTenantSlug &&
                        activeStaff.tenantSlug !==
                          activeStaff.originalTenantSlug && (
                          <p className="text-xs text-muted-foreground">
                            Moving this user to another pharmacy will remove
                            them from{" "}
                            {activeStaff.originalTenantName ??
                              activeStaff.originalTenantSlug}
                            .
                          </p>
                        )}

...
import { cashierLogin, enrollPosDevice, pinLogin } from "@/lib/services/auth";
import {
  createSale,
  getBatches,
  getCategories,
...
function PosSessionGate({ children }: { children: React.ReactNode }) {
  ...
  if (session === 1) return <PosPinGate onSuccess={refresh} onRebind={refresh} />;
  return <>{children}</>;
}
...
function PosPinGate({
  ...
  const submit = async () => {
    ...
    if (!id || pin.length < 4) {
      setError("Enter cashier ID and a PIN (at least 4 digits).");
      return;
    }
    setLoading(true);
    try {
      ...
      try {
        ...
        if (res.tenantSlug) localStorage.setItem("posTenantSlug", res.tenantSlug);
    ...
    >
      <div className="w-full max-w-sm space-y-6 rounded-2xl bg-white p-6 shadow-xl ring-0 dark:bg-slate-900">
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Cashier ID + PIN. Tenant is resolved from this device.
          </p>
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Cashier ID (from Staff page)"
            className="h-11 rounded-xl font-mono text-sm"
            autoComplete="off"
          />
        </div>
        <div className="space-y-2">
          <div className="flex h-12 items-center justify-center rounded-xl bg-[color:var(--pos-brand)]/10 font-mono text-2xl tracking-[0.4em] text-foreground shadow-inner">
            {pin ? (
              "●".repeat(pin.length)
            ) : (
...
function POSUserPageInner() {
  ...
  React.useEffect(() => {
    if (!heldSheetOpen && selectedReceipt == null) return;
  ...
  const handleLogout = React.useCallback(() => {
    ...
    // Full reload: we are already on `/`, so client `router.push` does not remount
    // `PosSessionGate` and the register UI would stay mounted without this.
  ...
  return (
    <div
      className="flex h-[calc(100dvh-3.5rem)] min-h-0 flex-col overflow-hidden bg-slate-100/80 font-sans text-slate-900 dark:bg-[#0c1816] dark:text-slate-100"
      style={{ ["--pos-brand" as string]: brand }}
    >
      <main className="flex min-h-0 flex-1 gap-5 overflow-hidden p-5 pt-5 sm:gap-6 sm:p-6 sm:pt-6">
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
          <div className="flex shrink-0 gap-1.5 rounded-xl bg-white p-1 shadow-sm dark:bg-slate-900/80">
            <Button
              type="button"
          ...
          {mainTab === "transactions" ? (
            <Card className="flex min-h-0 flex-1 flex-col overflow-hidden bg-white shadow-sm ring-0 dark:bg-slate-900/60">
              <CardHeader className="shrink-0 bg-[color:var(--pos-brand)]/[0.07] py-4 dark:bg-white/[0.04]">
                    ...
                    {transactions.map((tx) => (
                      <li key={`${tx.saleId ?? tx.receiptId}-${tx.createdAt}`}>
                        <button
                          ...
                          className="flex w-full items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3 text-left shadow-sm transition-colors hover:bg-[color:var(--pos-brand)]/10 dark:bg-slate-800/50 dark:hover:bg-slate-800"
                        >
          ...
          {mainTab === "register" ? (
            <>
              <Card className="shrink-0 gap-4 bg-white py-6 shadow-sm ring-0 dark:bg-slate-900/60">
              ...
              <div className="grid min-h-0 flex-1 grid-cols-2 gap-2 overflow-y-auto pr-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredProducts.length === 0 ? (
                  <div className="col-span-full flex min-h-[200px] items-center justify-center rounded-2xl bg-[color:var(--pos-brand)]/[0.06] px-4 py-12 text-center text-sm text-muted-foreground dark:bg-white/[0.04]">
                  ...
                  filteredProducts.map((p) => (
                    <Card
                      ...
                      className="group cursor-pointer gap-0 bg-white py-0 shadow-sm ring-0 transition-shadow hover:shadow-md dark:bg-slate-900/55"
                    >
        ...
        {mainTab === "register" ? (
          <Card className="flex w-[420px] shrink-0 flex-col overflow-hidden bg-white p-0 shadow-md ring-0 dark:bg-slate-900/60 p-3">
            <CardHeader className="flex flex-col gap-3 bg-[color:var(--pos-brand)]/[0.08] py-5 dark:bg-white/[0.05] p-3 rounded-lg">
              ...
              {checkoutStep === "cart" && (
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={holdOrder}
                    className="gap-1.5 p-2 border bg-[color:var(--pos-brand)]/12 font-semibold text-[color:var(--pos-brand)] hover:bg-[color:var(--pos-brand)]/18"
                  >
            ...
            {checkoutStep === "cart" ? (
              <>
                    ...
                    <>
                      ...
                      {cart.map((item) => {
                        ...
                        return (
                          <Card
                            key={item.lineId}
                            className="gap-0 bg-slate-50 py-3 ring-0 dark:bg-white/[0.06] "
                          >
                ...
                <CardFooter className="flex flex-col gap-4 bg-[color:var(--pos-brand)]/[0.08] py-6 dark:bg-white/[0.04] p-3 rounded-lg">
                  ...
                  <div className="grid w-full grid-cols-2 gap-3">
                    <Button
                      type="button"
                      variant="outline"
                      className="h-auto rounded-xl border-0 bg-[color:var(--pos-brand)]/12 py-4 font-bold text-[color:var(--pos-brand)] hover:bg-[color:var(--pos-brand)]/18"
                      onClick={clearCart}
                      disabled={cart.length === 0}
                    >
                      Cancel
              ...
              <CardFooter className="flex min-h-0 flex-1 flex-col gap-6 bg-[color:var(--pos-brand)]/[0.08] py-8 dark:bg-white/[0.04]">
                <div className="text-center">
                  ...
                  {PAYMENT_METHODS.map((m) => {
                    ...
                    return (
                      <Button
                        ...
                        type="button"
                        variant="outline"
                        className="h-auto flex-col gap-2 rounded-xl border-0 bg-white py-5 font-semibold shadow-sm hover:bg-[color:var(--pos-brand)]/12 dark:bg-slate-800/90"
        ...
        {selectedReceipt != null && typeof document !== "undefined"
          ? createPortal(
              <div
              ...
              >
                ...
                <aside
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="pos-transaction-receipt-title"
                  className="relative z-10 flex h-full w-full max-w-2xl flex-col border-l border-border bg-popover text-popover-foreground shadow-2xl"
                >
                  <div className="flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3">
                    <h2
                      id="pos-transaction-receipt-title"
                  ...
                  <p className="shrink-0 px-4 py-2 text-sm text-muted-foreground">
                    Preview matches the printed layout. Use Print receipt for
                    your printer.
                  </p>
                            ...
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex shrink-0 gap-2">
                                <Button
                                  ...
                                  size="sm"
                                  style={{ backgroundColor: brand }}
                                  className="text-primary-foreground p-2 rounded-lg cursor-pointer"
                                ...
                                <Button
                                  ...
                                  variant="ghost"
                                  size="sm"
                                  className="text-destructive hover:text-destructive p-2 rounded-lg cursor-pointer"
      ...
      <footer className="flex h-12 shrink-0 items-center justify-between bg-white px-8 text-[11px] text-muted-foreground shadow-[0_-10px_40px_-18px_rgba(15,23,42,0.1)] dark:bg-[#0c1816] dark:shadow-black/40">
        <div className="flex gap-6">
          <span className="flex items-center gap-1">
            <kbd className="rounded-md bg-slate-200/90 px-1.5 py-0.5 font-mono text-[10px] dark:bg-slate-700/90">
              F1
            </kbd>{" "}
            Search
          </span>
        ...
        <div className="flex items-center gap-4">
          <Button
            ...
            variant="outline"
            size="sm"
            className="h-8 gap-1.5 border-0 bg-slate-100 text-[11px] text-slate-700 hover:bg-[color:var(--pos-brand)]/14 dark:bg-slate-800 dark:text-slate-200"

...
/** Match {@link CurrencyEntryDialog} keypad footprint: 5 rows × 3 + OK column. */
const KEYS_ROWS: Array<Array<{ label: string; span?: number }>> = [
  [{ label: "1" }, { label: "2" }, { label: "3" }],
  [{ label: "4" }, { label: "5" }, { label: "6" }],
  [{ label: "7" }, { label: "8" }, { label: "9" }],
  [{ label: "0", span: 2 }, { label: "." }],
];

export function PercentageEntryDialog({
  open,
  onOpenChange,
  value,
  onValueChange,
          ...
          <div className="grid grid-cols-4 gap-1">
            <div className="col-span-3 grid grid-cols-3 gap-1">
              ...
              <Button
                type="button"
                variant="secondary"
                className="col-span-2 h-16 rounded-none border border-slate-300 bg-sky-100 text-xl font-extrabold tracking-tight text-slate-700 hover:bg-sky-200"
                onClick={() => append("00")}
              >
                00
              </Button>

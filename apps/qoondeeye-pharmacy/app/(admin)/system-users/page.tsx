  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) {
          setError(
            err instanceof Error ? err.message : "Failed to load system users",
          );
        }
      } finally {
        if (!cancelled) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formMode === "create") {
        ...
        const updated = await updateSystemUser(activeUser.id, {
                  ...
                  <div className="space-y-4 px-5 py-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleChange("role", value as EditableSystemUser["role"])
                        }
                      >
                    ...
                    <div className="rounded-lg border border-border/60 bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                      Roles are stored in the <code className="font-mono">super_admins</code>{" "}
                      table and sent by <code className="font-mono">/api/system-users</code>.
                    </div>
                  </div>

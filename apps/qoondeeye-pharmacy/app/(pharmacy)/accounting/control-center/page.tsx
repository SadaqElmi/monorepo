...
import {
  ...
  getVarianceAnalysis,
  type AlertItem,
  type ExplainNumberResult,
  type InterbranchMismatchItem,
  type InventoryGlSyncRow,
...
export default function ControlCenterPage() {
  ...
  const [mismatches, setMismatches] = React.useState<InterbranchMismatchItem[]>([]);
  const [inventorySync, setInventorySync] = React.useState<InventoryGlSyncRow[]>([]);
  ...
  const [variance, setVariance] = React.useState<VarianceAnalysisRow | null>(null);
  const [explain, setExplain] = React.useState<ExplainNumberResult | null>(null);
  const [healthRows, setHealthRows] = React.useState<
    Array<{ snapshotHour: string; checkKey: string; status: string }>
  >([]);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setMismatches(mismatchRes.items ?? []);
      setInventorySync(invRes.rows ?? []);
      setReadinessStatus(readinessRes.status);
      setAudit({
      ...
      setVariance(varianceRes.rows?.[0] ?? null);
      setExplain(explainRes);
      setHealthRows(
        (healthRes.items ?? []).map((row) => ({

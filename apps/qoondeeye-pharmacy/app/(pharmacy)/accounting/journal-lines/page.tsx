...
export default function JournalLinesPage() {
  ...
  const [rows, setRows] = React.useState<JournalLineFlatRow[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const data = await getJournalLines(tenantSlug, branchId, {
        from,
        to,
        limit: 1000,
      });
      ...
      setErr(e instanceof Error ? e.message : "Failed to load journal lines");
      setRows([]);
    } finally {
      setLoading(false);
    }
  }, [tenantSlug, branchId, from, to]);

...
import {
  ...
  type TransferMonitoringOverviewDto,
...
export default function TransferMonitoringPage() {
  ...
  const [overview, setOverview] = useState<TransferMonitoringOverviewDto | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setTenantSlug(user?.tenantSlug?.trim() || null);
  }, []);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const [nextOverview, branches] = await Promise.all([
        getTransferMonitoringOverview(tenantSlug),

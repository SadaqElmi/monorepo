...
export default function ConsolidatedReportsPage() {
  ...
  const [bs, setBs] = React.useState<ReportEnvelope<BalanceSheetResult> | null>(
    null,
  );
  const [pnl, setPnl] =
    React.useState<ReportEnvelope<IncomeStatementResult> | null>(null);
  const [loading, setLoading] = React.useState(false);
  const [err, setErr] = React.useState<string | null>(null);
  const [preview, setPreview] = React.useState<
    (ConsolidationPreviewResult & { scopeMeta?: unknown }) | null
  >(null);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : "Failed to load consolidated reports");
      setBs(null);
      setPnl(null);
    } finally {
      setLoading(false);
  ...
  React.useEffect(() => {
    ...
    setPreviewErr(null);
    void getConsolidationPreview(tenantSlug, asOf, branchId, aggregateAll)
      .then((p) => {
      ...
      .catch((e: unknown) => {
        if (!cancelled) {
          setPreview(null);
          setPreviewErr(
            e instanceof Error ? e.message : "Failed to load consolidation preview",
          );
        }
      })
      .finally(() => {
  ...
  }, [tenantSlug, asOf, branchId, aggregateAll, canMultiBranch]);

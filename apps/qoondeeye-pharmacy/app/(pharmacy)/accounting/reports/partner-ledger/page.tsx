...
export default function PartnerLedgerReportPage() {
  ...
  const [data, setData] = React.useState<PartnerLedgerResult | null>(null);
  ...
  const load = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await getPartnerLedger(
        tenantSlug,
        branchId,
        partnerKind,
        partnerId.trim(),
      ...
      setErr(e instanceof Error ? e.message : "Failed to load partner ledger");
      setData(null);
    } finally {
      setLoading(false);
    }
  }, [tenantSlug, branchId, aggregateAll, partnerKind, partnerId, from, to]);

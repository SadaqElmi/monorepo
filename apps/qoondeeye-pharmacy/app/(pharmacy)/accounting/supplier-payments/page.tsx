...
export default function SupplierPaymentsPage() {
  ...
  const [payments, setPayments] = React.useState<SupplierPaymentRow[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const [sup, pay] = await Promise.all([
        ...
        getSupplierPayments(tenantSlug, branchId, 80),
      ]);
      setSuppliers(sup);
      setPayments(pay);
    } catch (e: unknown) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!branchId) {
      setErr("Select a branch in the app header (or open Purchases once to set it).");
      return;
    }
    const amt = Number(amount);
    if (!supplierId || !Number.isFinite(amt) || amt <= 0) {
      setErr("Choose a supplier and enter a valid amount.");
      return;
    }
    setSubmitting(true);
    setErr(null);

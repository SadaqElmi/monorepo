...
export type BranchesPageClientProps = {
...
export default function BranchesPage({
  ...
  const branchesQuery = useErpBranches(tenantSlug, {
    initialData:
      serverPrefetched && initialBranches ? initialBranches : undefined,
  });
  ...
  const loadError =
    branchesQuery.error ?? inventoryQuery.error ?? staffQuery.error;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formMode === "create") {
        await createBranch(tenantSlug, payload);
      } else {
        await updateBranch(tenantSlug, activeBranch.id, payload);
      }

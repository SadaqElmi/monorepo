...
const USERS_NAV = [
  { label: "Staff & users", href: ROUTES.users.staff },
  { label: "Roles & permissions", href: ROUTES.users.roles },
] as const;
...
export type UsersModuleShellProps = {
  title: string;
  description: string;
...
export function UsersModuleShell({
  title,
  description,
  stat,
  headerEnd,
  children,
}: UsersModuleShellProps) {
  const StatIcon = stat?.icon;

  return (
    <ErpWorkbenchShell
      breadcrumbs={[
        { label: "Users", href: ROUTES.users.staff },
        { label: title },
      ]}
      headerEnd={headerEnd}
    >

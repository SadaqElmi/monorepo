...
export type JsonFetchOptions = RequestInit & {
  tenantSlug?: string;
  /** @internal one-shot retry after branch reconcile */
  _branchRetried?: boolean;
};
...
export async function jsonFetch<TResponse>(
  ...
  if (!res.ok) {
    const rawMessage = parseNestErrorMessage(data, "Request failed");
    if (!_branchRetried && typeof window !== "undefined") {
      const branchScopeMismatch =
        (res.status === 403 &&
          /access denied to this branch/i.test(rawMessage)) ||
        (res.status === 400 &&
          /valid branchid is required/i.test(rawMessage));
      if (branchScopeMismatch) {
        const user = getResolvedStoredUser();
        reconcileClientBranchSelection(user?.allowedBranchIds);
...
/** Download binary responses (exports, PDFs) with the same auth/branch headers. */
